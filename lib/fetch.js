...
FetchStream.prototype.drainBuffer = function () {
    ...
    if (this.responseBuffer.length > this.readyToRead) {
        push = new Buffer(this.readyToRead, 'binary');
        this.responseBuffer.copy(push, 0, 0, this.readyToRead);
        rest = new Buffer(this.responseBuffer.length - this.readyToRead, 'binary');
...
FetchStream.prototype.parseUrl = function (url) {
    ...
    if (urlparts.auth) {
        urloptions.headers.Authorization = 'Basic ' + new Buffer(urlparts.auth).toString('base64');
    }
